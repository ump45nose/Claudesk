...
container_id="$(docker compose ps --quiet claude-desktop)"
[ -n "$container_id" ] || {
    printf '%s\n' 'smoke: claude-desktop service is not running' >&2
    exit 1
}
...
claude_version="$(docker exec "$container_id" \
...
"$project_dir/scripts/chat-bridge-smoke.sh"
